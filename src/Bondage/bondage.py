...
from portedFiles.types import (BondageArgs, BondArgs, UnbondArgs, DelegateBondArgs, BondFilter,
                               Filter, txid, NetworkProviderOptions, Constants, NumType, TransactionCallback)
...
class ZapBondage(BaseContract):
    ...
    async def bond(self, bond_args: BondArgs, callback: TransactionCallback):
        ...
        if broker != const.NULL_ADDRESS:
            if bond_args['frm'] != broker:
                raise Error(
                    'Broker address {broker} needs to call delegate bonding'.format(broker))
        promiEvent = self.contract.bond(
            bond_args['provider'],
            utf8ToHex(bond_args['endpoint']), dots).send({bond_args['frm'], bond_args['gas'], bond_args['gasPrice']})
        if callback:
            # Add events
            pass
        return promiEvent
    ...
    async def delegateBond(self, delegate_bond_args: DelegateBondArgs, callback: TransactionCallback):
        ...
        if broker != const.NULL_ADDRESS:
            if delegate_bond_args['frm'] != broker:
                raise Error(
                    'Broker address {broker} needs to call delegate bonding for this endpoint'.format(broker))
        promiEvent = self.contract.delegateBond(
            ...
            dots).send({delegate_bond_args['frm'], delegate_bond_args['gas'], delegate_bond_args['gasPrice']})
        if callback:
            # Add events
            pass
        return promiEvent
    ...
    async def unbond(self, unbond_args: UnbondArgs, callback: TransactionCallback):
        ...
        if broker != const.NULL_ADDRESS:
            if unbond_args['frm'] != broker:
                raise Error(
                    'Broker address {broker} needs to call unbonding for this endpoint'.format(broker))
        promiEvent = self.contract.unbond(
            unbond_args['provider'],
            utf8ToHex(unbond_args['endpoint']),
            dots).send({unbond_args['frm'], unbond_args['gas'], unbond_args['gasPrice']})
        if callback:
            # Add events
            pass
        return promiEvent
    ...
    def listen(callback: TransactionCallback, filters: BondFilter = {}):
        self.contract.events.allEvents(
            filters, {'fromBlock': 0, 'toBlock': 'latest'}, callback)

    def listenBound(callback: TransactionCallback, filters: BondFilter = {}):
        self.contract.events.Bound(filters, {'toBlock': 'latest'}, callback)

    def listenBound(callback: TransactionCallback, filters: BondFilter = {}):
        self.contract.events.Unbond(filters, {'toBlock': 'latest'}, callback)

    def listenEscrowed(callback: TransactionCallback, filters: BondFilter = {}):
        self.contract.events.Escrowed(filters, {'toBlock': 'latest'}, callback)

    def listenReleased(callback: TransactionCallback, filters: BondFilter = {}):
        self.contract.events.Released(filters, {'toBlock': 'latest'}, callback)

    def listenReturned(callback: TransactionCallback, filters: BondFilter = {}):
